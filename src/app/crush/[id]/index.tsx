...
export default function CrushChat() {
  ...
  const submit = (text: string) => {
    const current = image;
    setImage(null);
    send({ crushId: crush.id, image: current, text, tone });
  };
  ...
  return (
    <View style={[styles.root, { backgroundColor: theme.background }]}>
        ...
        ListEmptyComponent={
          <View style={styles.empty}>
            ...
            <View style={styles.starterRow}>
              {STARTERS.map((s) => (
                <Pressable key={s} accessibilityRole="button" onPress={() => send({ crushId: crush.id, image: null, text: s, tone })} style={[styles.starter, { backgroundColor: theme.surface }]}>
      ...
      <Composer tone={tone} onToneChange={setTone} image={image} onPickImage={chooseImage} onClearImage={() => setImage(null)} onSend={submit} sending={sending} />
