...
export default function ApiKeySettings() {
  ...
  const save = async () => {
    ...
    if (trimmed && !trimmed.startsWith('sk-ant-')) {
      toast.show('Anthropic API 키는 sk-ant- 로 시작해요.', 'error');
      return;
    }
  ...
  return (
    <Screen keyboard contentStyle={styles.content}>
      {APP_CONFIG.apiUrl ? (
        ...
        <Card tone="accent" style={styles.status}>
          ...
          <View style={styles.statusTexts}>
            ...
            <AppText variant="small" color="textSecondary">
              Anthropic 콘솔에서 발급한 키를 넣으면 내 계정으로 코칭 요청이 이루어져요. 키는 기기의 보안 저장소(키체인)에만 저장돼요.
            </AppText>
          </View>
        </Card>
      )}
      ...
      <TextField
        ...
        secureTextEntry
        editable={loaded}
        helper="console.anthropic.com → API Keys 에서 발급할 수 있어요."
      />
      <Button title="저장" onPress={save} disabled={!loaded} />
      <Button title="API 키 발급 페이지 열기" variant="ghost" onPress={() => Linking.openURL('https://console.anthropic.com/settings/keys')} />
    </Screen>
  );
}
